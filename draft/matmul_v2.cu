...
__global__ void sgemm_v2(float* A, float* B, float* C, int M, int N, int K, float alpha, float beta){
    ...
    for (int k = 0; k < K; k += BK){
        As[ty][tx] = A[OFFSET(global_y, k + tx, K)];
        Bs[ty][tx] = B[OFFSET(k + ty, global_x, N)];
...
void call_v2(float* A, float* B, float* C, int M, int N, int K){
    ...
    sgemm_v2<32><<<gridsize, blocksize>>>(A, B, C, M, N, K, alpha, beta);
...
float* v2_time(float* h_A, float* h_B, float* h_C, int M, int N, int K){
    int size_A = M * K;
    int size_B = N * K;
    int size_C = M * N;
    ...
    std::cout << "v2 time: " << elapse_time << "ms" << std::endl;
...
int main(){
    ...
    for (int size:sizes){
        ...
        h_C_sgemm = v2_time(h_A, h_B, h_C_sgemm, M, N, K);
