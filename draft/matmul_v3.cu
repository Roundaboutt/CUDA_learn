...
__global__ void sgemm_v3(float* A, float* B, float* C, int M, int N, int K, float alpha, float beta){
    ...
    const int a_tile_row = linear_tid / BK;
    const int a_tile_col = linear_tid % BK;
    const int a_tile_stride = num_threads / BK;

    const int b_tile_row = linear_tid / BN;
    const int b_tile_col = linear_tid % BN;
    ...
    for (int k = 0; k < K; k += BK){
        ...
        for (int k1 = 0; k1 < BK; k1++){
            ...
            for (int i = 0; i < TM; i++){
                ...
                #pragma unroll
                for (int j = 0; j < TN; j++){
                    temp[i][j] += As[ty + i][k1] * Bs[k1][tx + j];
                }
            }
        }
    ...
    for (int i = 0; i < TM; i++){
        ...
        #pragma unroll
        for (int j = 0; j < TN; j++){
            C[OFFSET(ty + i, tx + j, N)] = alpha * temp[i][j] + beta * C[OFFSET(ty + i, tx + j, N)];
        }
    }
}
